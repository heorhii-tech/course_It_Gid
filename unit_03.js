...
function t4() {
    let r_4 = document.querySelectorAll('.r-4')


    for (let i = 0; i < r_4.length; i++) {
        if (r_4[i].checked) {
            return r_4[i].value
        }
...
function t14() {
    return `rgb(${t13(0, 255)},${t13(0, 255)},${t13(0, 255)})`.replace()
}
...
function t17() {
    let s_171 = document.querySelector('.s-171').value;
    return s_171
...
function t26() {
    let i_26 = document.querySelector('.i-26').value;
    if (i_26 === ' ') {
        return
    } else {
        document.querySelector('.out-26').textContent = i_26.trim()
    }
}
...
document.querySelector('.b-28').addEventListener('click', function () {
    document.querySelector('.out-28').textContent = t28('hello my world', 'ry');
})
